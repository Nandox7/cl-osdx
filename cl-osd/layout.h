...
if (textId == 0) {  
		...
		printText(gText[textId], pos, "\152"); //Battery symbol + '1'
		//pos += 7;
		pos += 11;
#endif
    ...
    printText(gText[textId], pos, "\145"); //Antenna symbol (RSSI)
	pos += 7;
#endif
...
#endif //TIME_ENABLED
*/
	printText(gText[textId], TEXT_LINE_MAX_CHARS - 6, "\155"); // Clock symbol
...
*/
#ifdef TIME_HOUR_ENABLED
	pos = printTime(gText[textId], TEXT_LINE_MAX_CHARS - 13); // Time with hours
#else
    pos = printTime(gText[textId], TEXT_LINE_MAX_CHARS - 10); // Time without hours
#endif //TIME_HOUR_ENABLED
  }
